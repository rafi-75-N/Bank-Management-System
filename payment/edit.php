<?php require_once __DIR__ . '/../config/db.php'; ?>
<?php
$id = $_GET['id'] ?? null;
if (!$id) { die('Missing id'); }
$rec = $conn->query("SELECT * FROM payment WHERE PaymentID=".(int)$id)->fetch_assoc();
if (!$rec) { die('Record not found'); }

$errors = [];
if ($_SERVER['REQUEST_METHOD'] === 'POST') {

    $stmt = $conn->prepare("UPDATE payment SET Amount=?, PaymentDate=?, LoanID=? WHERE PaymentID=?");
    $Amount = $_POST['Amount'] ?? null;
    $PaymentDate = $_POST['PaymentDate'] ?? null;
    $LoanID = $_POST['LoanID'] ?? null;
    $stmt->bind_param('dsii', $Amount, $PaymentDate, $LoanID, $id);
    if ($stmt->execute()) {
        header("Location: /banking_system/payment/list.php");
        exit;
    } else {
        $errors[] = "Update failed: " . $stmt->error;
    }
}
?>
<?php include __DIR__ . '/../includes/header.php'; ?>
<h3 class="mb-3 text-capitalize">Edit payment</h3>
<?php if (!empty($errors)) { echo '<div class="alert alert-danger">'.implode('<br>', $errors).'</div>'; } ?>
<form method="post" class="card p-3 shadow-sm">
  <div class="row g-3">

    <div class="col-md-6">
      <label class="form-label">Amount</label>
      <input type="number" name="Amount" class="form-control" value="<?php echo htmlspecialchars($rec['Amount']); ?>" step="0.01" required>
    </div>

    <div class="col-md-6">
      <label class="form-label">PaymentDate</label>
      <input type="date" name="PaymentDate" class="form-control" value="<?php echo htmlspecialchars($rec['PaymentDate']); ?>" required>
    </div>

    <div class="col-md-6">
      <label class="form-label">LoanID (Loan)</label>
      <select name="LoanID" class="form-select" required>
        <option value="">-- select --</option>
        <?php
        $q = $conn->query("SELECT LoanID, LoanID FROM Loan ORDER BY LoanID");
        while ($o = $q->fetch_assoc()) {
            $sel = ($rec['LoanID'] == $o['LoanID']) ? 'selected' : '';
            echo '<option '+$sel+' value="'.htmlspecialchars($o['LoanID']).'">'.htmlspecialchars($o['LoanID']).' (ID: '.htmlspecialchars($o['LoanID']).')</option>';
        }
        ?>
      </select>
    </div>

  </div>
  <div class="mt-3">
    <button class="btn btn-primary">Update</button>
    <a href="list.php" class="btn btn-secondary">Cancel</a>
  </div>
</form>
<?php include __DIR__ . '/../includes/footer.php'; ?>
